#include "Galaxy.hpp"
 ...
 galaxy::GalacticCore::GalacticCore(config::Galaxy&& config)
  : game(std::make_unique<pg::game::Game>())
  , galaxyConfig(std::move(config))
{
}
...
{
    auto& scene = game->createAndSwitchScene("start");
...
void galaxy::GalacticCore::setupConfig()
{
...
void galaxy::GalacticCore::setupSelectionMarker()
{
    ...
    entt::entity markers = pg::game::makeEntity<pg::Transform2D, pg::game::Drawable, pg::tags::DebugRenderingItemTag>(
...
void galaxy::GalacticCore::setupQuadtreeDebug()
{
...
void galaxy::GalacticCore::setupOverlay()
{
    ...
    pg::game::makeEntity<pg::game::GuiDrawable>(
        game->getRegistry(), {std::make_unique<galaxy::gui::DashBoardWidget>(*game), pg::game::DRAWABLE_DOCKING_AREA});
...
void galaxy::GalacticCore::setupGalaxy()
{
    ...
    // add some background
    auto background_sprite = game->getTypedResourceCache<pg::Sprite>().load("../data/background/milky_way_blurred.png");
